...
import com.telegrambot.appointment.management.model.appointment.*;
import com.telegrambot.appointment.management.model.appointment.booking.BookingContext;
import com.telegrambot.appointment.management.model.appointment.booking.BookingPath;
import com.telegrambot.appointment.management.model.appointment.booking.BookingStep;
...
import com.telegrambot.appointment.management.repository.appointment.*;
import com.telegrambot.appointment.management.repository.user.ClientRepository;
import com.telegrambot.appointment.management.repository.user.SpecialistRepository;
...
public class AppointmentBookingService {
    ...
    private SendMessage handleConfirmation(BookingContext context, Long chatId, String data) {
        ...
        Specialist specialist = specialistRepository.findById(context.getSelectedSpecialistId())
                .orElseThrow();
        Service service = serviceRepository.findById(context.getSelectedServiceId())
                .orElseThrow();
        ScheduleSlot slot = slotRepository.findById(context.getSelectedSlotId())
                .orElseThrow();
    ...
    private SendMessage showServices(Long chatId) {
        ...
        if (services.isEmpty()) {
            return new SendMessage(chatId.toString(), "Нет доступных услуг.");
        }
    ...
    private SendMessage showAllSpecialists(Long chatId) {
        ...
        if (specialists.isEmpty()) {
            return new SendMessage(chatId.toString(), "Нет доступных специалистов.");
        }
    ...
    private SendMessage showServicesBySpecialist(Long chatId, Integer specialistId) {
        ...
        if (services.isEmpty()) {
            return new SendMessage(chatId.toString(), "У специалиста нет услуг.");
        }
    ...
    private SendMessage showAvailableSlots(Long chatId, Integer scheduleId) {
        ...
        if (slots.isEmpty()) {
            return new SendMessage(chatId.toString(),
                    "В этот день нет свободных слотов. Выберите другой день.");
        }
